...
class CheckPasswordInHistoryListener extends AbstractListener {
    ...
    public function on_entry_point(Event $event = null) {
        ...
        if ($password->checkPasswordExists($params['CustomerAuthorization']['password'], $member['passwordHistory'])) {
            setSession('ERROR_RESULT', array('CustomerAuthorization' => array('password' => 'VALIDATION_PASSWORD_IN_HISTORY')));
