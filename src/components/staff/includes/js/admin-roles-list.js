...
$(document).ready(function () {
        ...
        $("#dialog-confirm").dialog({
            ...
            buttons: {
                "Delete item": function () {
                    $.post('/super/staff/roles/remove/' + id);
                    $(this).dialog("close");
                    $('#row_' + id).remove();
                },
                Cancel: function () {
    ...
    function save() {
        $.post("/super/staff/roles/" + $('#Role_id').val(), $("#form1").serialize())
                .success(function (data) {
                    if ($('#Role_id').val() == 0) {
                        addRow(data.StaffRole[0].id, data.StaffRole[0].role, data.StaffRole[0].title);
                    }
