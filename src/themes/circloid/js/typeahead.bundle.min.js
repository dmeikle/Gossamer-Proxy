...
(function (b) {
    var q = function () {
        return{isMsie: function () {
            ...
            }, isArray: b.isArray, isFunction: b.isFunction, isObject: b.isPlainObject, isUndefined: function (u) {
                return typeof u === "undefined"
            }, toStr: function t(u) {
                return q.isUndefined(u) || u === null ? "" : u + ""
            }, bind: b.proxy, each: function (w, u) {
                b.each(w, v);
                function v(x, y) {
                    return u(y, x)
                }}
            , map: b.map, filter: b.grep, every: function (v, w) {
                var u = true;
                if (!v) {
                    return u
                }
                b.each(v, function (x, y) {
                    if (!(u = w.call(null, y, x, v))) {
                        return false
                    }
                });
                return !!u
                ...
                return !!u
            }, mixin: b.extend, getUniqueId: function () {
                var u = 0;
                return function () {
                    return u++
                }
            }(), templatify: function s(v) {
                return b.isFunction(v) ? v : u;
                function u() {
                    return String(v)
                }}
            , defer: function (u) {
                setTimeout(u, 0)
            }, debounce: function (w, y, v) {
                var x, u;
                return function () {
                    var C = this, B = arguments, A, z;
                    A = function () {
                        x = null;
                        if (!v) {
                            u = w.apply(C, B)
    ...
    var l = "0.10.5";
    var n = function () {
        return{nonword: s, whitespace: t, obj: {nonword: u(s), whitespace: u(t)}};
        function t(v) {
            v = q.toStr(v);
            return v ? v.split(/\s+/) : []
        }
    ...
    var d = function () {
        function t(A) {
            this.maxSize = q.isNumber(A) ? A : 100;
            this.reset();
            if (this.maxSize <= 0) {
                this.set = this.get = b.noop
            }
        }
        q.mixin(t.prototype, {set: function z(A, D) {
        ...
        q.mixin(w.prototype, {add: function y(A) {
                if (this.head) {
                    A.next = this.head;
                    this.head.prev = A
                }
    ...
    var o = function () {
        ...
        function v(z) {
            this.prefix = ["__", z, "__"].join("");
            this.ttlKey = "__ttl__";
            this.keyMatcher = new RegExp("^" + q.escapeRegExChars(this.prefix))
        }
        if (s && window.JSON) {
            t = {_prefix: function (z) {
                    return this.prefix + z
                }, _ttlKey: function (z) {
                    return this._prefix(z) + this.ttlKey
                }, get: function (z) {
                    if (this.isExpired(z)) {
                        this.remove(z)
                    }
                    ...
                    if (q.isNumber(z)) {
                        s.setItem(this._ttlKey(A), w(u() + z))
                    } else {
                        s.removeItem(this._ttlKey(A))
                    }
                    return s.setItem(this._prefix(A), w(B))
                }, remove: function (z) {
                    s.removeItem(this._ttlKey(z));
                    s.removeItem(this._prefix(z));
                    return this
                }, clear: function () {
                    var B, A, C = [], z = s.length;
                    for (B = 0; B < z; B++) {
                        if ((A = s.key(B)).match(this.keyMatcher)) {
                            C.push(A.replace(this.keyMatcher, ""))
                        }
                    }
                    for (B = C.length; B--; ) {
                        this.remove(C[B])
                    ...
                    var z = y(s.getItem(this._ttlKey(A)));
                    return q.isNumber(z) && u() > z ? true : false
                }}
        } else {
            t = {get: q.noop, set: q.noop, remove: q.noop, clear: q.noop, isExpired: q.noop}
        }
        ...
        function w(z) {
            return JSON.stringify(q.isUndefined(z) ? null : z)
        }
        function y(z) {
            return JSON.parse(z)
        }}
    ...
    var p = function () {
        ...
        z.setMaxPendingRequests = function t(A) {
            s = A
        };
        z.resetCache = function y() {
            v.reset()
        };
        q.mixin(z.prototype, {_get: function (E, H, B) {
                var G = this, F;
                if (this.cancelled || E !== this.lastUrl) {
                    return
                }
                if (F = w[E]) {
                    F.done(D).fail(C)
    ...
    var i = function () {
        function s(B) {
            ...
            if (!B.datumTokenizer || !B.queryTokenizer) {
                b.error("datumTokenizer and queryTokenizer are both required")
            }
            this.datumTokenizer = B.datumTokenizer;
            this.queryTokenizer = B.queryTokenizer;
            this.reset()
        }
        q.mixin(s.prototype, {bootstrap: function u(B) {
        ...
        return s;
        function w(B) {
            B = q.filter(B, function (C) {
                return !!C
            });
            B = q.map(B, function (C) {
    ...
    ();
    var f = function () {
        return{local: u, prefetch: s, remote: t};
        function u(v) {
            return v.local || null
        }
        function s(x) {
            ...
            if (w = x.prefetch || null) {
                w = q.isString(w) ? {url: w} : w;
                w = q.mixin(v, w);
        ...
        function t(z) {
            ...
            function v(A) {
                return function (B) {
                    return q.debounce(B, A)
                }
            }
            function w(A) {
                return function (B) {
                    return q.throttle(B, A)
                }
            }}
    }
    ();
    (function (D) {
        ...
        function u(L) {
            if (!L || !L.local && !L.prefetch && !L.remote) {
                b.error("one of local, prefetch, or remote is required")
            }
        ...
        u.noConflict = function K() {
            D.Bloodhound = s;
            return u
        };
        u.tokenizers = n;
        q.mixin(u.prototype, {_loadPrefetch: function v(P) {
                var M = this, O, L;
                if (O = this._readFromStorage(P.thumbprint)) {
                    this.index.bootstrap(O);
                    L = b.Deferred().resolve()
                } else {
                    L = b.ajax(P.url, P.ajax).done(N)
                }
                return L;
                function N(Q) {
                    M.clear();
            ...
            , _getFromRemote: function x(Q, L) {
                ...
                function M(R, S) {
                    R ? L([]) : L(P.remote.filter ? P.remote.filter(S) : S)
                }}
            , _cancelLastRemoteRequest: function J() {
                this.transport && this.transport.cancel()
            }, _saveToStorage: function E(N, M, L) {
                if (this.storage) {
                    this.storage.set(y.data, N, L);
                    this.storage.set(y.protocol, location.protocol, L);
                    this.storage.set(y.thumbprint, M, L)
                }
            }, _readFromStorage: function I(M) {
                var L = {}, N;
                if (this.storage) {
                    L.data = this.storage.get(y.data);
            ...
            , clear: function C() {
                this.index.reset()
            }, clearPrefetchCache: function w() {
                this.storage && this.storage.clear()
            }, clearRemoteCache: function F() {
                this.transport && p.resetCache()
            }, ttAdapter: function H() {
                return q.bind(this.get, this)
            }});
        return u;
        function B(M) {
            return q.isFunction(M) ? L : N;
            function L(O) {
                return O.sort(M)
            }
            function N(O) {
    ...
    var g = function () {
        var s = {wrapper: {position: "relative", display: "inline-block"}, hint: {position: "absolute", top: "0", left: "0", borderColor: "transparent", boxShadow: "none", opacity: "1"}, input: {position: "relative", verticalAlign: "top", backgroundColor: "transparent"}, inputWithNoHint: {position: "relative", verticalAlign: "top"}, dropdown: {position: "absolute", top: "100%", left: "0", zIndex: "100", display: "none"}, suggestions: {display: "block"}, suggestion: {whiteSpace: "nowrap", cursor: "pointer"}, suggestionChild: {whiteSpace: "normal"}, ltr: {left: "0", right: "auto"}, rtl: {left: "auto", right: " 0"}};
        if (q.isMsie()) {
            q.mixin(s.input, {backgroundImage: "url(data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)"})
        }
        if (q.isMsie() && q.isMsie() <= 7) {
            q.mixin(s.input, {marginTop: "-1px"})
        }
        return s
    }();
    var a = function () {
        ...
        function s(u) {
            if (!u || !u.el) {
                b.error("EventBus initialized without el")
            }
            this.$el = b(u.el)
        }
        q.mixin(s.prototype, {trigger: function (v) {
    ...
    var e = function () {
        ...
        function A(G, E, C, D) {
            ...
            while (F = E.shift()) {
                this._callbacks[F] = this._callbacks[F] || {sync: [], async: []};
        ...
        function w(D) {
            ...
            while ((E = D.shift()) && (F = this._callbacks[E])) {
                G = x(F.sync, this, [E].concat(C));
        ...
        function x(F, E, D) {
            ...
            function C() {
                var I;
                for (var H = 0, G = F.length; !I && H < G; H += 1) {
                    I = F[H].apply(E, D) === false
                }
                return !I
            }}
        function s() {
        ...
        function B(D, C) {
            return D.bind ? D.bind(C) : function () {
    ...
    var k = function () {
        ...
        function L(X) {
            ...
            if (this.$hint.length === 0) {
                this.setHint = this.getHint = this.clearHint = this.clearHintIfInvalid = q.noop
            }
            if (!q.isMsie()) {
                this.$input.on("input.tt", T)
            } else {
                this.$input.on("keydown.tt keypress.tt cut.tt paste.tt", function (Y) {
                    if (O[Y.which || Y.keyCode]) {
                        return
                    }
        ...
        L.normalizeQuery = function (S) {
            return(S || "").replace(/^\s*/g, "").replace(/\s{2,}/g, " ")
        };
        q.mixin(L.prototype, e, {_onBlur: function C() {
                this.resetInputValue();
                this.trigger("blurred")
            }, _onFocus: function w() {
                this.trigger("focused")
            }, _onKeydown: function Q(S) {
                var T = O[S.which || S.keyCode];
                this._managePreventDefault(T, S);
                if (T && this._shouldTrigger(T, S)) {
                    this.trigger(T + "Keyed", S)
                }
            }, _onInput: function t() {
                this._checkInputValue()
            }, _managePreventDefault: function y(W, V) {
                var U, T, S;
                switch (W) {
                    case"tab":
                        T = this.getHint();
                ...
                return this.query
            }, setQuery: function I(S) {
                this.query = S
            }, getInputValue: function x() {
                return this.$input.val()
            }, setInputValue: function A(T, S) {
                this.$input.val(T);
            ...
            }, setHint: function N(S) {
                this.$hint.val(S)
            }, clearHint: function D() {
                this.setHint("")
            }, clearHintIfInvalid: function s() {
                var V, U, S, T;
                ...
                !T && this.clearHint()
            }, getLanguageDirection: function G() {
                return(this.$input.css("direction") || "ltr").toLowerCase()
            }, hasOverflow: function K() {
                var S = this.$input.width() - 2;
        ...
        return L;
        function E(S) {
            return b('<pre aria-hidden="true"></pre>').css({position: "absolute", visibility: "hidden", whiteSpace: "pre", fontFamily: S.css("font-family"), fontSize: S.css("font-size"), fontStyle: S.css("font-style"), fontVariant: S.css("font-variant"), fontWeight: S.css("font-weight"), wordSpacing: S.css("word-spacing"), letterSpacing: S.css("letter-spacing"), textIndent: S.css("text-indent"), textRendering: S.css("text-rendering"), textTransform: S.css("text-transform")}).insertAfter(S)
        }
        function B(T, S) {
            return L.normalizeQuery(T) === L.normalizeQuery(S)
        }
        function F(S) {
            return S.altKey || S.ctrlKey || S.metaKey || S.shiftKey
        }}
    ();
    var r = function () {
        var t = "ttDataset", H = "ttValue", B = "ttDatum";
        function s(I) {
            I = I || {};
            I.templates = I.templates || {};
            if (!I.source) {
                b.error("missing source")
            }
            if (I.name && !v(I.name)) {
                b.error("invalid dataset name: " + I.name)
            }
            this.query = null;
        ...
        s.extractDatasetName = function F(I) {
            return b(I).data(t)
        };
        s.extractValue = function C(I) {
            return b(I).data(H)
        };
        s.extractDatum = function C(I) {
            return b(I).data(B)
        };
        q.mixin(s.prototype, e, {_render: function u(O, I) {
                if (!this.$el) {
                    return
                }
                var N = this, K;
                this.$el.empty();
                K = I && I.length;
                if (!K && this.templates.empty) {
                    this.$el.html(J()).prepend(N.templates.header ? M() : null).append(N.templates.footer ? L() : null)
                } else {
                    if (K) {
                        this.$el.html(P()).prepend(N.templates.header ? M() : null).append(N.templates.footer ? L() : null)
                    }
                }
                this.trigger("rendered");
                function J() {
                    return N.templates.empty({query: O, isEmpty: true})
                }
                function P() {
                    var R, Q;
                    ...
                    function S(T) {
                        ...
                        U.children().each(function () {
                            b(this).css(g.suggestionChild)
                        });
                        return U
                    }}
                function M() {
                    return N.templates.header({query: O, isEmpty: !K})
                }
                function L() {
                    return N.templates.footer({query: O, isEmpty: !K})
                }}
            , getRoot: function D() {
                return this.$el
            }, update: function y(K) {
        ...
        function x(J, K) {
            return{empty: J.empty && q.templatify(J.empty), header: J.header && q.templatify(J.header), footer: J.footer && q.templatify(J.footer), suggestion: J.suggestion || I};
            function I(L) {
                return"<p>" + K(L) + "</p>"
            }}
        function v(I) {
    ...
    var m = function () {
        function L(U) {
            ...
            q.each(this.datasets, function (V) {
                S.$menu.append(V.getRoot());
                V.onSync("rendered", S._onRendered, S)
            })
        }
        q.mixin(L.prototype, e, {_onSuggestionClick: function K(Q) {
                this.trigger("suggestionClicked", b(Q.currentTarget))
            }, _onSuggestionMouseEnter: function s(Q) {
                this._removeCursor();
                this._setCursor(b(Q.currentTarget), true)
            }, _onSuggestionMouseLeave: function F() {
                this._removeCursor()
            }, _onRendered: function y() {
                this.isEmpty = q.every(this.datasets, Q);
            ...
            , _hide: function () {
                ...
                if (Q < 0) {
                    this.$menu.scrollTop(R + Q)
                } else {
                    if (U < S) {
                        this.$menu.scrollTop(R + (S - U))
                    }
                }
            }, close: function G() {
                if (this.isOpen) {
            ...
            }, setLanguageDirection: function O(Q) {
                this.$menu.css(Q === "ltr" ? g.ltr : g.rtl)
            }, moveCursorUp: function z() {
                this._moveCursor(-1)
            }, moveCursorDown: function P() {
                this._moveCursor(+1)
            }, getDatumForSuggestion: function B(R) {
                var Q = null;
                if (R.length) {
                    Q = {raw: r.extractDatum(R), value: r.extractValue(R), datasetName: r.extractDatasetName(R)}
                }
                return Q
            }, getDatumForCursor: function w() {
                return this.getDatumForSuggestion(this._getCursor().first())
            }, getDatumForTopSuggestion: function I() {
                return this.getDatumForSuggestion(this._getSuggestions().first())
            }, update: function A(Q) {
                q.each(this.datasets, R);
                function R(S) {
                    S.update(Q)
                }}
            , empty: function D() {
    ...
    var j = function () {
        ...
        q.mixin(D.prototype, {_onSuggestionClicked: function u(Z, Y) {
                var X;
                if (X = this.dropdown.getDatumForSuggestion(Y)) {
                    this._select(X)
                }
            }, _onCursorMoved: function O() {
                var X = this.dropdown.getDatumForCursor();
                this.input.setInputValue(X.value, true);
                this.eventBus.trigger("cursorchanged", X.raw, X.datasetName)
            }, _onCursorRemoved: function x() {
                this.input.resetInputValue();
                this._updateHint()
            }, _onDatasetRendered: function J() {
                this._updateHint()
            }, _onOpened: function v() {
                this._updateHint();
                this.eventBus.trigger("opened")
            }, _onClosed: function A() {
                this.input.clearHint();
                this.eventBus.trigger("closed")
            }, _onFocused: function L() {
                this.isActivated = true;
                this.dropdown.open()
            }, _onBlurred: function M() {
                this.isActivated = false;
                this.dropdown.empty();
                this.dropdown.close()
            }, _onEnterKeyed: function H(Y, X) {
                var Z, aa;
                ...
                if (Z) {
                    ...
                    if (this.autoselect && aa) {
                        this._select(aa);
                        X.preventDefault()
                    }
                }
            }, _onTabKeyed: function W(Z, Y) {
                var X;
                if (X = this.dropdown.getDatumForCursor()) {
                    this._select(X);
                    Y.preventDefault()
                } else {
                    this._autocomplete(true)
                }
            }, _onEscKeyed: function R() {
                this.dropdown.close();
                this.input.resetInputValue()
            }, _onUpKeyed: function t() {
                var X = this.input.getQuery();
                ...
                this.dropdown.open()
            }, _onLeftKeyed: function V() {
                this.dir === "rtl" && this._autocomplete()
            }, _onRightKeyed: function I() {
                this.dir === "ltr" && this._autocomplete()
            }, _onQueryChanged: function F(Y, X) {
                this.input.clearHintIfInvalid();
                X.length >= this.minLength ? this.dropdown.update(X) : this.dropdown.empty();
                this.dropdown.open();
                this._setLanguageDirection()
            }, _onWhitespaceChanged: function S() {
                this._updateHint();
                this.dropdown.open()
            }, _setLanguageDirection: function Q() {
                var X;
                if (this.dir !== (X = this.input.getLanguageDirection())) {
                    this.dir = X;
                    this.$node.css("direction", X);
                    this.dropdown.setLanguageDirection(X)
                }
            }, _updateHint: function K() {
                var Y, ac, ab, aa, X, Z;
                Y = this.dropdown.getDatumForTopSuggestion();
                if (Y && this.dropdown.isVisible() && !this.input.hasOverflow()) {
                    ac = this.input.getInputValue();
            ...
            }, _select: function G(X) {
                this.input.setQuery(X.value);
                this.input.setInputValue(X.value, true);
                this._setLanguageDirection();
                ...
                X = q.toStr(X);
                if (this.isActivated) {
                    this.input.setInputValue(X)
                } else {
                    this.input.setQuery(X);
        ...
        function N(X, ac) {
            ...
            try {
                !ad.attr("dir") && ad.attr("dir", "auto")
            } catch (aa) {
            }
            return ad.wrap(Z).parent().prepend(ac ? Y : null).append(ab)
        }
        function z(X) {
            return{backgroundAttachment: X.css("background-attachment"), backgroundClip: X.css("background-clip"), backgroundColor: X.css("background-color"), backgroundImage: X.css("background-image"), backgroundOrigin: X.css("background-origin"), backgroundPosition: X.css("background-position"), backgroundRepeat: X.css("background-repeat"), backgroundSize: X.css("background-size")}
        }
        function w(X) {
            var Y = X.find(".tt-input");
            q.each(Y.data(T), function (aa, Z) {
                q.isUndefined(aa) ? Y.removeAttr(Z) : Y.attr(Z, aa)
            });
    ...
    (function () {
        ...
        u = b.fn.typeahead;
        s = "ttTypeahead";
        v = {initialize: function w(D, C) {
                C = q.isArray(C) ? C : [].slice.call(arguments, 1);
                D = D || {};
                return this.each(B);
                function B() {
                    var G = b(this), E, F;
                    q.each(C, function (H) {
                        H.highlight = !!D.highlight
                    });
            ...
            , val: function t(B) {
                return !arguments.length ? D(this.first()) : this.each(C);
                function C() {
                    var F = b(this), E;
                    if (E = F.data(s)) {
                        E.setVal(B)
        ...
        b.fn.typeahead = function (C) {
            ...
            if (v[C] && C !== "initialize") {
                ...
                return v.initialize.apply(this, arguments)
            }
        };
        b.fn.typeahead.noConflict = function y() {
            b.fn.typeahead = u;
            return this
        }
    })()
